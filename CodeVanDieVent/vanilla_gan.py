...
def training_loop(train_dataloader, opts):
    ...
    # Loss function:
    loss_criterion = torch.nn.MSELoss(reduce = True)
    if torch.cuda.is_available():
        loss_criterion.cuda()
...
#            print(D(real_images))
            D_real_loss = loss_criterion(D(real_images), ones_label[:real_images.size()[0]])
            ...
            D_fake_loss = loss_criterion(D(fake_images), zeros_label[:fake_images.size()[0]])
            ...
            G_loss = loss_criterion(D(fake_images), ones_label[:fake_images.size()[0]])
