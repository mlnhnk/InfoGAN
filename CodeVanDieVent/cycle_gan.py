...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts):
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        D_X_loss = loss_criterion(D_X(images_X), ones_label[:images_X.size()[0]])
        D_Y_loss = loss_criterion(D_Y(images_Y), ones_label[:images_Y.size()[0]])
        ...
        fake_X = G_XtoY(images_X)

        # 3. Compute the loss for D_X
        D_X_loss = loss_criterion(D_Y(fake_X), zeros_label[:fake_X.size()[0]])

        # 4. Generate fake images that look like domain Y based on real images in domain X
        fake_Y = G_YtoX(images_Y)

        # 5. Compute the loss for D_Y
        D_Y_loss = loss_criterion(D_X(fake_Y), zeros_label[:fake_Y.size()[0]])
        ...
        g_loss = loss_criterion(D_Y(fake_X), ones_label[:fake_X.size()[0]])

        if opts.use_cycle_consistency_loss:
            reconstructed_Y = G_XtoY(fake_X)
            # 3. Compute the cycle consistency loss (the reconstruction loss)
            cycle_consistency_loss = loss_criterion(images_Y, G_XtoY(fake_X))
        ...
        g_loss = loss_criterion(D_X(fake_Y), ones_label[:fake_Y.size()[0]])

        if opts.use_cycle_consistency_loss:
            reconstructed_X = G_YtoX(fake_Y)
            # 3. Compute the cycle consistency loss (the reconstruction loss)
            cycle_consistency_loss = loss_criterion(images_X, G_YtoX(fake_Y))
            g_loss += cycle_consistency_loss
...
def create_parser():
    ...
    parser.add_argument('--train_iters', type=int, default=1500, help='The number of training iterations to run (you can Ctrl-C out earlier if you want).')
